...
export default function ProductionPage() {
  ...
  const fetchDeals = async () => {
    ...
    const productionDeals = Array.isArray(data)
      ? data.filter((d: Deal) => ['production', 'dispatch_ready'].includes(d.stage))
      : []
    setDeals(productionDeals)
    setLoading(false)
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 mt-1">Active manufacturing orders</p>
      </div>
      ...
      {deals.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-400">No active production orders</p>
        </div>
      ) : (
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {delayed > 0 && (
                          <span className="flex items-center gap-1 text-xs text-red-600 bg-red-50 px-2 py-0.5 rounded-full">
                            <AlertCircle className="w-3 h-3" /> {delayed} delayed
                          </span>
                        )}
                        {deal.stage === 'dispatch_ready' && (
                          <span className="text-xs text-green-600 bg-green-50 px-2 py-0.5 rounded-full">Ready to Dispatch</span>
                        )}
                      </div>
                      <div className="flex items-center gap-2 text-xs text-gray-400 mt-0.5">
                        <span className="font-mono">{deal.dealNumber}</span>
                        {deal.material && <span>· {deal.material.toUpperCase()}</span>}
                        {deal.motorType && <span>· {deal.motorType.toUpperCase()}</span>}
                        {deal.expectedDispatch && <span>· Expected: {formatDate(deal.expectedDispatch)}</span>}
                      </div>
                    </div>
                    <Link href={`/dashboard/deals/${deal.id}`}>
                      <Button variant="outline" size="sm">View Deal <ArrowRight className="w-3 h-3" /></Button>
                    </Link>
                  </div>
                </CardHeader>
                <CardContent>
                  ...
                  <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-11 gap-2">
                    {stages.map((stage: any) => {
                      ...
                      return (
                        <div
                          key={stage.id}
                          className={`flex flex-col items-center p-2 rounded-lg text-center cursor-default ${
                            stage.status === 'completed' ? 'bg-green-50 border border-green-200' :
                            stage.status === 'in_progress' ? 'bg-blue-50 border border-blue-200' :
                            stage.status === 'delayed' ? 'bg-red-50 border border-red-200' :
                        ...
                        >
                          <div className={`w-6 h-6 rounded-full flex items-center justify-center mb-1 ${
                            ...
                            {stage.status === 'completed' ? <Check className="w-3 h-3 text-white" /> :
                             stage.status === 'in_progress' ? <Activity className="w-3 h-3 text-white" /> :
                             stage.status === 'delayed' ? <AlertCircle className="w-3 h-3 text-white" /> :
                             <Clock className="w-3 h-3 text-gray-400" />}
                          </div>
                          <p className="text-xs text-gray-600 leading-tight" style={{ fontSize: '10px' }}>
                            {meta?.label || stage.stageName}
                          </p>
                          <div className="mt-1 flex gap-1">
                            {stage.status === 'pending' && (
                              <button onClick={() => updateStage(stage.id, 'in_progress')} className="text-blue-500 hover:text-blue-700 text-xs">▶</button>
                            )}
                            {stage.status === 'in_progress' && (
                              <button onClick={() => updateStage(stage.id, 'completed')} className="text-green-500 hover:text-green-700 text-xs">✓</button>
                            )}
                          </div>
                        </div>
                      )
                  ...
                  {inProgress && (
                    <div className="mt-3 flex items-center gap-2 text-sm">
                      ...
                      <span className="font-medium text-blue-700">
                        {PRODUCTION_STAGES.find(p => p.key === inProgress.stageName)?.label || inProgress.stageName}
                      </span>
                      {inProgress.actualStart && (
                        <span className="text-xs text-gray-400">· Started {formatDate(inProgress.actualStart)}</span>
                      )}
                    </div>
                  )}
                </CardContent>
