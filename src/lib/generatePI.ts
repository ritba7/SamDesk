...
export function generatePI(deal: any, piNumber?: string): void {
  ...
  const itemDesc = [
    'Air Shower',
    deal.dealNumber ? `Model: ${deal.dealNumber}` : '',
    outerDims ? `Size (OD): ${outerDims}` : '',
    innerDims ? `Size (ID): ${innerDims}` : '',
    deal.material ? `Material: ${deal.material.toUpperCase()}` : '',
    motorStr ? `Motor: ${motorStr}` : '',
  ].filter(Boolean).join('\n')
  ...
  const itemRows: any[] = [
    ...
    [
      ...
      { content: fmt(basicPrice), styles: { fontStyle: 'bold' } },
    ],
  ]
  ...
  if (isSameState) {
    ...
    itemRows.push([
      { content: 'IGST @ 18%', colSpan: 5, styles: { halign: 'right' } },
      { content: fmt(gstAmount) },
    ])
  }
